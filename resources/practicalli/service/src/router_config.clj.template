...
(ns {{top/ns}}.{{main/ns}}.router
  ...
  (:require
   ...
   ;; Service middleware
   [{{top/ns}}.{{main/ns}}.middleware :as middleware-service]

   ;; Service Routing
   [{{top/ns}}.{{main/ns}}.api.system-admin :as system-admin]
   [{{top/ns}}.{{main/ns}}.api.scoreboard   :as scoreboard]
...
(def open-api-docs
  ...
  ["/swagger.json"
   {:get {:no-doc  true
          :swagger {:info {:title "{{top/ns}} {{main/ns}} Service API"
                           :description "{{description}}"
                           :version "0.1.0"
                           :termsOfService "https://{{group/id}}/"
                           :contact {:name "Engineering Team"
                                     :url "https://{{group/id}}/wiki/engineering-team"}
                           :license {:name "© {{developer}}, {{now/year}}"
